...
function loadCredentials(): ServiceAccount | undefined {
  ...
  try {
    ...
    if (typeof parsed?.project_id !== "string") return undefined;
...
try {
  ...
  if (creds) {
    initializeApp({
      ...
      storageBucket: `${(creds as Record<string, string>).project_id ?? (creds as Record<string, string>).projectId}.appspot.com`,
    });
  } else {
    initializeApp();
  }
...
type ReqLike = { method?: string; headers?: { authorization?: string }; body?: unknown; query?: Record<string, string | string[] | undefined> };

function getToken(req: ReqLike): string | null {
  const authHeader = req.headers?.authorization;
  const bearer = typeof authHeader === "string" ? authHeader.replace(/^Bearer\s+/i, "").trim() : "";
  if (bearer) return bearer;
  const body = req.body && typeof req.body === "object" && "token" in req.body ? (req.body as { token?: string }).token : null;
...
async function requireAdmin(req: ReqLike): Promise<{ uid: string } | null> {
...
export const session = onRequest(
  { cors: true },
  async (req, res) => {
...
export const eventos = onRequest(
  { cors: true },
  async (req, res) => {
    if (req.method === "OPTIONS") {
    ...
    if (req.method === "POST") {
      const admin = await requireAdmin(req as ReqLike);
      if (!admin) {
    ...
    if (req.method === "DELETE") {
      ...
      const id = typeof req.query?.id === "string" ? req.query.id : (req.body && typeof req.body === "object" && "id" in req.body ? (req.body as { id?: string }).id : null);
      if (!id) {
...
export const informacion = onRequest(
  { cors: true },
  async (req, res) => {
    if (req.method === "OPTIONS") {
      res.set(CORS_HEADERS).status(204).send();
    ...
    if (req.method === "POST") {
      ...
      try {
        await db.collection("informacion").doc(INFO_DOC).set(
          { title: title.trim(), body: content.trim(), updatedAt: FieldValue.serverTimestamp() },
          { merge: true }
        );
        res.set(CORS_HEADERS).status(200).json({ success: true });
      } catch (e) {
...
export const oracion = onRequest(
  { cors: true },
  async (req, res) => {
    if (req.method === "OPTIONS") {
      res.set(CORS_HEADERS).status(204).send();
    ...
    if (req.method === "GET") {
      try {
        ...
        res.set(CORS_HEADERS).status(200).json(
          data ?? { verseText: "", verseRef: "", message: "" }
        );
      } catch (e) {
        res.set(CORS_HEADERS).status(500).json({ error: "read_failed" });
      }
    ...
    if (req.method === "POST") {
      ...
      try {
        await db.collection("oracion").doc(ORACION_DOC).set(
          { verseText: verseText.trim(), verseRef: verseRef.trim(), message: message.trim(), updatedAt: FieldValue.serverTimestamp() },
          { merge: true }
        );
        res.set(CORS_HEADERS).status(200).json({ success: true });
      } catch (e) {
...
export const multimedia = onRequest(
  { cors: true },
  async (req, res) => {
    if (req.method === "OPTIONS") {
      res.set(CORS_HEADERS).status(204).send();
...
export const ministerios = onRequest(
  { cors: true },
  async (req, res) => {
    if (req.method === "OPTIONS") {
      res.set(CORS_HEADERS).status(204).send();
    ...
    if (req.method === "POST") {
      ...
      try {
        if (id) {
          await db.collection("ministerios").doc(id).set(
            { title: title.trim(), text: text.trim(), icon: iconVal, updatedAt: FieldValue.serverTimestamp() },
            { merge: true }
          );
  ...
  async (req, res) => {
    ...
    if (req.method === "POST") {
      ...
      const slots = slotsRaw
        .filter((s: unknown) => s && typeof s === "object" && "day" in s && "event" in s && "hour" in s)
        .map((s: { day?: string; event?: string; hour?: string; icon?: string }) => ({
      ...
      const porDefinir = porDefinirRaw
        ...
        .map((x: string) => x.trim());
      try {
        await db.collection("agenda").doc(AGENDA_DOC).set(
          { slots, porDefinir, updatedAt: FieldValue.serverTimestamp() },
          { merge: true }
        );
        res.set(CORS_HEADERS).status(200).json({ success: true });
      } catch (e) {
